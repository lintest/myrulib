...
void FbTreeViewHeaderWindow::CheckSize(wxDC & dc, int width)
{
	...
	if (width < fixedWidth + floatWidth) {
		fixed_x = (double) width / (fixedSize + floatSize);
		float_x = fixed_x;
	} else {
		fixed_x = cw;
		float_x = (double) (width - fixedWidth) / floatSize;
	}

...
// Created:     06/01/06
// RCS-ID:      $Id: mymodels.cpp 59471 2009-03-10 14:06:04Z RR $
...
{
    m_root = new MyMusicTreeModelNode( NULL, _("My Music" );

    // setup pop music
    m_pop = new MyMusicTreeModelNode( m_root, _("Pop music" );
    m_pop->Append(
        new MyMusicTreeModelNode( m_pop, _("You are not alone"), _("Michael Jackson"), 1995 ) );
    m_pop->Append(
        new MyMusicTreeModelNode( m_pop, _("Take a bow"), _("Madonna"), 1994 ) );
    m_root->Append( m_pop );

    // setup classical music
    m_classical = new MyMusicTreeModelNode( m_root, _("Classical music" );
    m_ninth = new MyMusicTreeModelNode( m_classical, _("Ninth symphony"), _("Ludwig van Beethoven"), 1824 );
    m_classical->Append( m_ninth );
    m_classical->Append( new MyMusicTreeModelNode( m_classical, _("German Requiem"), _("Johannes Brahms"), 1868 ) );
                                 ...
                                 const wxDataViewItem &item, unsigned int col ) const
{
    ...
    default:
        wxLogError( _("MyMusicTreeModel::GetValue: wrong column %d"), col );
    }
}
                                 ...
                                 const wxDataViewItem &item, unsigned int col )
{
    ...
    switch (col)
    {
        ...
        default:
            wxLogError( _("MyMusicTreeModel::SetValue: wrong column") );
    }
    return false;
}
        ...
        wxDataViewVirtualListModel( INITIAL_NUMBER_OF_ITEMS )
{
    ...
    {
        wxString str;
        str.Printf( _("real row %d"), i );
        m_array.Add( str );
    }

    m_icon[0] = wxIcon( wxT(null_xpm) );
    m_icon[1] = wxIcon( wxT(wx_small_xpm) );
}
...
void MyListModel::DeleteItems( const wxDataViewItemArray &items )
{
    ...
    if (rows.GetCount() == 0)
    {
        ...
        wxLogError( _("Cannot remove rows with an index greater than %d"), m_array.GetCount() );
        return;
    }
                                 ...
                                 unsigned int row, unsigned int col ) const
{
    ...
    {
        if (row >= m_array.GetCount())
            variant = wxString::Format( _("virtual row %d"), row );
        else
            variant = m_array[ row ];
    }
    else if (col==1)
    {
        wxDataViewIconText data( _("test"), m_icon[ row%2 ] );
        variant << data;
    }
    else if (col==2)
    {
                                 ...
                                 unsigned int row, unsigned int col )
{
    if (col == 0)
    {
        if (row >= m_array.GetCount())
        {
            ...
            wxLogError( _("Cannot edit rows with an index greater than %d"), m_array.GetCount() );
            return false;
        }
